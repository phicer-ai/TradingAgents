...
仓库内 CodeGraph 就绪资产：
- 配置：`projects/backend/TradingAgents/.codegraph/config.json`。
- Wrapper：`projects/backend/TradingAgents/scripts/codegraph.mjs`。
- 轻量生成输出：`projects/backend/TradingAgents/.codegraph/out/codegraph.json` 和 `projects/backend/TradingAgents/.codegraph/out/codegraph.md`。
- 外部 CodeGraph CLI 的 SQLite 数据库：`projects/backend/TradingAgents/.codegraph/codegraph.db`。

SQLite 数据库和生成输出文件属于本地生成产物。刷新方式：
...
此仓库目前没有仓库内 MCP server、Codex skill 或 generated-agent 输出目录。如果以后新增，请放在 `projects/backend/TradingAgents` 下，并在这里和 `projects/backend/TradingAgents/AGENTS.md` 中记录调用方式。
